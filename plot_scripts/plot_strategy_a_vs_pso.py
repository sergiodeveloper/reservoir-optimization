...
pso_path = os.path.join(DATA_DIR, 'RegrasPSO_npv-wei.csv')

# Load data
runs516 = pd.read_csv(runs516_path)
pso_data = pd.read_csv(pso_path)
...
for well in wells:
    ...
    # PSO data (blue)
    pso_weif_values = pso_data['WEIF[' + well + ']'].values
    ...
    pso_df = pd.DataFrame({
        'WEIF': pso_weif_values,
        'Execution': 'PSO'
    })

    merged = pd.concat([pso_df, strategy_a_df]).reset_index(drop=True)
...
# Main title
fig.suptitle(
    "WEIF distribution comparison: Proposed method vs PSO (Particle Swarm Optimization)",
    fontsize=14
)
...
output_file_png = os.path.join(OUTPUT_DIR, 'strategy_a_vs_pso_comparison.png')
output_file_pdf = os.path.join(OUTPUT_DIR, 'strategy_a_vs_pso_comparison.pdf')
...
for well in wells:
    ...
    pso_data_well = pso_data['WEIF[' + well + ']']
    ...
    print(f"  PSO - Count: {len(pso_data_well)}")
    if len(pso_data_well) > 0:
        print(f"    Minimum: {pso_data_well.min():.2e}")
        print(f"    Maximum: {pso_data_well.max():.2e}")
        print(f"    Mean: {pso_data_well.mean():.2e}")
        print(f"    Median: {pso_data_well.median():.2e}")
